...
public class BinaryDepthMapExtract : MonoBehaviour
{
    ...
    {
        string name = objectToPrefabMap[obj.name];
        return name != null? name : "";
    }

    bool WriteFrameInfo(Transform current, int depth)
    {
        bool isPrefab = false;
        ...
        isPrefab = WritePrefabName("<ModelName>", current);
    ...
    private bool WritePrefabName(string strHeader, Transform current)
    {
        var name = GetPrefabName(current.gameObject);
        if (name == "") return false;
        binaryWriter.Write(strHeader);
        Debug.Log($"Object Name : {current.name} ,Prefab Name: {name}");
        WriteString(name);
        return true;
    }

    void WriteFrameHierarchyInfo(Transform child, int depth)
    {
        bool isPrefab = WriteFrameInfo(child, depth);

        if((isPrefab && depth < Maxdepth) || false == isPrefab)
        {
            ...
            if (child.childCount > 0)
            {
                ...
                {
                    WriteFrameHierarchyInfo(child.GetChild(k), depth + 1);
                }
            }
        }
    ...
    void Start()
    {
        ...
        binaryWriter = new BinaryWriter(File.Open("Extract Model/" + string.Copy(gameObject.name).Replace(" ", "_") + ".bin", FileMode.Create));

		WriteString("<Hierarchy>:");
        WriteFrameHierarchyInfo(transform, 0);
		WriteString("</Hierarchy>");
