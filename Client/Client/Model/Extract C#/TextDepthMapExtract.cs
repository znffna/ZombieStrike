...
public class TextDepthMapExtract : MonoBehaviour
{
    ...
    static public Dictionary<string, string> objectToPrefabMap = new Dictionary<string, string>();
   ...
   void WriteString(string strToWrite)
    ...
    {
        string prefabName = objectToPrefabMap[obj.name];
        if(prefabName == null) prefabName = "";

        Debug.Log($"{obj.name} → Prefab: {prefabName}");

        //return prefabRoot != null ? prefabRoot.name : "";
        return prefabName;
    }

    bool WritePrefabName(int nLevel, string strHeader, Transform current)
    {
        var name = GetPrefabName(current.gameObject);
    ...
    {
        bool isPrefab = WriteFrameInfo(nLevel, current);

       if(false == isPrefab || (isPrefab && nLevel <= (Maxdepth * 2)))
    ...
    void OnValidate()  // This runs in Edit Mode, before Play Mode
    {
        if (!Application.isPlaying)  // We only care about capturing it in Edit Mode
        {
            GameObject mine = this.gameObject;
            // Get the Prefab path before play mode
            foreach (GameObject obj in GameObject.FindObjectsOfType<GameObject>())
            {
                ...
                if (mine == obj)
                {
                    objectToPrefabMap[obj.name] = "";
                    continue;
                }

                bool isSelfPurePrefab =
             PrefabUtility.GetPrefabInstanceStatus(obj) == PrefabInstanceStatus.Connected &&
             PrefabUtility.GetNearestPrefabInstanceRoot(obj) == obj;

                //Debug.Log($"{obj.name} Prefab Path captured: {prefabPath}");
                if(isSelfPurePrefab)
                {  
                    string prefabName = Path.GetFileNameWithoutExtension(prefabPath);
                    //Debug.Log($"Prefab Name: {prefabName}");

                    objectToPrefabMap[obj.name] = prefabName;
                }
                else
                {
                    objectToPrefabMap[obj.name] = "";
                    print($"Prefab Name: {obj.name} is not pure prefab");
                }
            }
        }
    }
    ...
    void Start()
    {
        print("Model Text Write Start");
        m_pSkinnedMeshRenderers = GetComponentsInChildren<SkinnedMeshRenderer>();
        print(m_pSkinnedMeshRenderers.Length);
        for (int i = 0; i < m_pSkinnedMeshRenderers.Length; i++)
        {
            m_pSkinnedMeshRenderers[i].forceMatrixRecalculationPerRender = true;
        }

        m_rTextWriter = new StreamWriter("Extract Model/" + string.Copy(gameObject.name).Replace(" ", "_") + ".txt");
      ...
      //  {
      //      print("Model Text Write Completed" + m_raAnimationClips.Length + "\n");
