...
#define ROOT_PARAMETER_CAMERA (ROOT_PARAMETER_MATERIAL + 1)		// 2
#define ROOT_PARAMETER_FRAMEWORK (ROOT_PARAMETER_CAMERA + 1)	// 3
#define ROOT_PARAMETER_LIGHT (ROOT_PARAMETER_FRAMEWORK + 1)		// 4

#ifdef _WITH_STANDARD_TEXTURE_MULTIPLE_PARAMETERS
// Standard Texture (SRV)
#define ROOT_PARAMETER_STANDARD_TEXTURES (ROOT_PARAMETER_LIGHT + 1)		// 5
#define ROOT_PARAMETER_ALBEDO_TEXTURE (ROOT_PARAMETER_STANDARD_TEXTURES)	// 5
#define ROOT_PARAMETER_SPECULAR_TEXTURE (ROOT_PARAMETER_ALBEDO_TEXTURE + 1) // 6
#define ROOT_PARAMETER_NORMAL_TEXTURE (ROOT_PARAMETER_SPECULAR_TEXTURE + 1) // 7
#define ROOT_PARAMETER_METALLIC_TEXTURE (ROOT_PARAMETER_NORMAL_TEXTURE + 1) // 8
#define ROOT_PARAMETER_EMISSION_TEXTURE (ROOT_PARAMETER_METALLIC_TEXTURE + 1) // 9
#define ROOT_PARAMETER_DETAIL_ALBEDO_TEXTURE (ROOT_PARAMETER_EMISSION_TEXTURE + 1) // 10
#define ROOT_PARAMETER_DETAIL_NORMAL_TEXTURE (ROOT_PARAMETER_DETAIL_ALBEDO_TEXTURE + 1) // 11
// SkyCube Texture
#define ROOT_PARAMETER_SKYBOX (ROOT_PARAMETER_DETAIL_NORMAL_TEXTURE + 1) // 12		
#else
// Standard Texture (SRV)
#define ROOT_PARAMETER_STANDARD_TEXTURES (ROOT_PARAMETER_LIGHT + 1)		// 5
// SkyCube Texture
...
// Direct3D 12 - Device Descriptor Increment Size
...
extern ComPtr<ID3D12Resource> CreateTextureResourceFromWICFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, const wchar_t* pszFileName, ID3D12Resource** ppd3dUploadBuffer, D3D12_RESOURCE_STATES d3dResourceStates = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
extern ComPtr<ID3D12Resource> CreateTexture2DResource(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, UINT nWidth, UINT nHeight, UINT nElements, UINT nMipLevels, DXGI_FORMAT dxgiFormat, D3D12_RESOURCE_FLAGS d3dResourceFlags, D3D12_RESOURCE_STATES d3dResourceStates, D3D12_CLEAR_VALUE* pd3dClearValue);
...
namespace Plane
{
	...
	inline XMFLOAT4 GetPlaneFromTransform(const XMFLOAT4X4& xmf4Transform)
	{
		// Transform Matrix를 XMMATRIX 형식으로 변환
		XMMATRIX matrix = XMLoadFloat4x4(&xmf4Transform);

		// 평면의 법선 벡터 (Transform의 Z축을 사용 - Look Vector)
		XMVECTOR forward = XMVector3Normalize(matrix.r[2]);

		// 평면의 한 점 (Transform의 위치를 사용)
		XMVECTOR position = matrix.r[3];

		// 법선 벡터의 X, Y, Z를 가져옴
		XMFLOAT3 normal;
		XMStoreFloat3(&normal, forward);

		// 평면 방정식의 D값 계산: -dot(normal, position)
		float d = -XMVectorGetX(XMVector3Dot(forward, position));
